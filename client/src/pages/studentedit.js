...
const StudentEditProfile = () => {
  ...
  useEffect(() => {
    const fetchStudentData = async ()=>{
        try{
            const res = await axios.get("http://localhost:8800/students/3")
            console.log(res)
...
const handleSaveChanges = async () => {
  try {
    ...
    const response = await axios.put("http://localhost:8800/users/3", updatedData);
...
const handleUploadFile = async () => {
  ...
  try {
    if (selectedFile) {
      ...
      const response = await axios.post(`http://localhost:8800/users/profile_picture/3`, formData, {
  headers: {
